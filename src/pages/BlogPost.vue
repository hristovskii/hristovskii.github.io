<template>
  <section class="relative z-20 mx-auto max-w-4xl px-4 py-12 sm:py-16">
    <nav class="mb-4 rounded-md  p-3" aria-label="Breadcrumb">
    ...
    <div v-if="post">
      <header class="mb-6">
        <h1 class="text-4xl font-extrabold mb-2">{{ post.title }}</h1>
      ...
      <footer class="mt-12 border-t pt-6 flex items-center justify-between gap-4">
        <div>
          <router-link
            v-if="prevPost"
            :to="`/blog/${prevPost.slug}`"
            class="inline-flex items-center gap-2 px-3 py-2 border rounded-md text-sm hover:bg-gray-100 dark:hover:bg-gray-800 transition"
